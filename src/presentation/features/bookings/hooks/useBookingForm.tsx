...
const setTotalsByDates = (formik: any, entryDate: string, exitDate: string) => {
  ...
  formik.setFieldValue(
    "total_reservation",
    days * (formik.values.total_rooms ?? 0),
  );
};
...
export const useBookingForm = ({
  ...
  const getRoomsAvailable = useCallback(
    async (dates?: IEntryExitDates): Promise<IRoomAvailability | undefined> => {
      ...
      const { entry_date, exit_date } =
        dates ?? useBookingStore.getState().values;
      ...
      try {
        ...
        const resp =
          await bookingRepository.getRoomAvailability<IRoomAvailability>(
            params,
          );
    ...
    if (typeof respListRooms !== "undefined") {
      ...
      const rowlist = rooms.map((room: any) => ({
        ...
        price_options: [
          {
            code: room.val_min,
            name: `Valor min: ${formatCurrency(room.val_min)}`,
          },
          {
            code: room.val_max,
            name: `Valor max: ${formatCurrency(room.val_max)}`,
          },
        ],
        room_options:
  ...
  const handleSave = useCallback(
    ({
      values,
      setLoading,
    }: IPropsSave<IBookings & { rooms_reservations: any[] }>) => {
      const data = { ...values };
        ...
        .then((resp) => {
          ...
          if (customer.length > 0) {
            ...
            formik.setFieldValue(
              ...
              customer[0].cell_phone_emergency,
            );
