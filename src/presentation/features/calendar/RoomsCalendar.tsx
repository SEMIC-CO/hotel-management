...
export const RoomsCalendar = () => {
  ...
  /* Derived */
  const days = useMemo(
    () => buildDayRange(startDate, VISIBLE_DAYS),
    [startDate],
  );
  ...
  const stats = useMemo(() => {
    ...
    const checkinsToday = reservations.filter((r) =>
      dayjs(r.entry_date).isSame(dayjs(), "day"),
    ).length;
  ...
  return (
    <div className="w-full p-4">
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          <p className="text-sm text-gray-700">
            Vista operativa en tiempo real – {MONTH_NAMES[dayjs().month()]}{" "}
            {dayjs().year()}
          </p>
        </div>
        <Button
      ...
      {/* Filters + date nav */}
      <div className="flex flex-wrap items-center gap-3 mb-4 font-light">
        <div className="flex gap-2">
          <Button
            label="TODAS"
            size="small"
            className={`${activeFilter === "Todas" ? "" : "p-button-outlined"} text-white font-ligth`}
          ...
          {roomTypes.map((t) => (
            <Button
              ...
              label={t}
              size="small"
              className={`${activeFilter === t ? "" : "p-button-outlined"} text-white`}
        ...
        <div className="flex items-center gap-2 ml-auto">
          <Button
            icon="pi pi-chevron-left text-white"
            text
            size="small"
            onClick={prevRange}
          />
          ...
          <Button
            icon="pi pi-chevron-right text-white"
            text
            size="small"
            onClick={nextRange}
          />
      ...
      <div className="flex flex-wrap gap-4 mt-4 text-xs text-gray-600">
        <LegendItem
          color={LEGEND_COLORS.INGRESADO}
          label="Ocupada / Ingresado"
        />
        <LegendItem
          color={LEGEND_COLORS.RESERVADA}
          label="Reserva Confirmada"
        />
        <LegendItem
          color={LEGEND_COLORS.PENDIENTE_CONFIRMAR}
